...
def printUsage():
   ...
   print 'e.g.'
   print '/opt/spark/bin/spark-submit --main mesos://zk://mesos_main_01:2181,mesos_main_02:2181,mesos_main_03:2181/mesos --driver-memory 512M --executor-memory 2G --total-executor-cores 8 %s "testProc" "/mnt/nfs/test/eric/lte/set_*_100/set_00*.txt" "lte_eric_schema.json" "/mnt/nfs/test/cellLookup.pqz" "/mnt/nfs/test/out.pqz" "/mnt/nfs/test/testLte" \'{"overwrite":false, "partitionNum":2, "loadFactor":3}\' \n' % (curr_py_filename)
...
def loadCellLookup(lookupPQ):
   ...
   # read lookup parquet
   newlookupPQ = lookupPQ + "/TECH=%s/VENDOR=%s" % ('LTE','Ericsson')
...
def addPkAndSaveParquet(origDF, writemode, outputDir, numPartition=None):
   ...
   df = spark.sql("SELECT k.*, IFNULL(l.MARKET,'unassigned') as HL_Market, IFNULL(l.CLUSTER,'unassigned') AS HL_Cluster, IFNULL(l.AREA,'unassigned') AS HL_Area from kpi k left join lookup l on UPPER(k.OSSName) = UPPER(l.OSS) AND k.EUtranCellFDD = l.CELL")
