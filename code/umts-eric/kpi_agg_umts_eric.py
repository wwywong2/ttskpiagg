...
# argv[1] - process name
# argv[2] - input file (txt) e.g. "/mnt/nfs/test_results/eric/umts/set_*-*/set_001.txt"; if empty, skip to next stage
...
# argv[5] - output parquet dir e.g. "/mnt/nfs/test/westest_umts.pqz"
# argv[6] - output csv e.g. "/mnt/nfs/test/westest_umts"; if empty, not creating export
...
def printUsage():
   ...
   print '   inSchemaFile - schema file (json) (empty if not needed) e.g. "umts_eric_schema.json"'
   print '   inLookupPQ   - input cell lookup parquet (mandatory) e.g. "/mnt/nfs/test/cellLookup.pqz"'
   print '   outPQ        - output parquet dir (mandatory) e.g. "/mnt/nfs/test/outUmts.pqz"'
   print '   outCSV       - output csv in tgz package (w/o extension);  e.g. "/mnt/nfs/test/umtsExport";'
...
def csvToParquet1(spark, inputCsv, schema, outputDir, numPartition=None, overwrite=False):
   ...
   if (archiveDir != ''): # if cannot create archive folder, don't do anything
      ...
      # e.g. ttskpiagg_ERICSSON_UMTS_20161020_123347123_TMO.txt
...
def aggKPI1(spark, pq, jsonFile, workdir):
   ...
   # get folder name e.g. /mnt/nfs/test/ttskpiagg_ERICSSON_UMTS_TMO_20161020123347123
   ...
   if len(csv1arr) < 4: 
      ...
      # e.g. ttskpiagg_ERICSSON_UMTS_20161020_123347123_TMO
...
/*ORDER BY pk_date,pk_market,pk_hr,HL_Site,HL_UMTSSort,UtranCell*/ " % (sqlStr)
   ...
   for market,marketItem in dateItem.items():
      ...
      for hour,hourItem in sorted(marketItem.items()): # key2: hour; value: pathname
         ...
         if sqlDF.count() > 0:
            # e.g.  /mnt/nfs/test/ttskpiagg_ERICSSON_UMTS_TMO_20161020123347123/
            #       ttskpiagg_ERICSSON_UMTS_20161020_123347123_TMO_2016-09-10_nyc_LONG-ISLAND_09.csv
      ...
      if len(marketItem.items()) > 0: # if there is any hr data, create daily data
         ...
         # save df to csv
         # e.g.  /mnt/nfs/test/ttskpiagg_ERICSSON_UMTS_TMO_20161020123347123/
         #       ttskpiagg_ERICSSON_UMTS_20161020_123347123_TMO_2016-09-10_nyc_LONG-ISLAND.csv
         saveCsv(sqlDF, workdir + '/' + csv1 + "_%s_%s_%s.csv" % (
		date, suffix, market.replace(' ', '-')))


      # zip to market file
      # e.g.  workdir - /mnt/nfs/test/ttskpiagg_ERICSSON_UMTS_TMO_20161020123347123/
      #       workdir1 - /mnt/nfs/test/
      #       outCSVTgz - ttskpiagg_ERICSSON_UMTS_20161020_123347123_TMO_2016-09-10_nyc_LONG-ISLAND_22-8c6502a4-6b60-44fc-a097-ceb9c4ca1ff1.tgz
...
def main(spark,inCSV,outPQ,outCSV):
   ...
   try:
      ...
      if inCSV is not "":
         ...
         # note: default(null) partition - 2G exec mem - 4 cores (2 exec) - 20 union - ok
         # note: default(null) partition - 2G exec mem - 8 cores (4 exec) - 20 union - ok
